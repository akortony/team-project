...
app.post("/login", (req, res) => {
    ...
    db.query(sql, [username], async (err, results) => {
        console.log("Login Attempt - Results:", results.length);
        if (err) {
        ...
        // Generate JWT Token
        const token = jwt.sign({ id: doctor.id, username: doctor.username }, process.env.JWT_SECRET, { expiresIn: "1h" });
        
        console.log("Login Attempt - res:", res);
